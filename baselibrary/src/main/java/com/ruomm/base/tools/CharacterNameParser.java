...
public class CharacterNameParser {
	...
	private static CharacterNameParser characterParser = new CharacterNameParser();
	public static final String ValueType_ENG = "(en-us) ";
	public static final String ValueType_CHS = "(zh-cn) ";
	public static final String ValueUnknown = "|";

	public static CharacterNameParser getInstance() {
	...
	public String getSelling(String chs) {
		if (StringUtils.isEmpty(chs)) {
			return " " + CharacterNameParser.ValueType_ENG;
		}

		buffer = new StringBuilder();
		String firstKey = chs.substring(0, 1);
		if (TextUtils.isEmpty(firstKey)) {
			buffer.append(" " + CharacterNameParser.ValueType_ENG);
		}

		else if (firstKey.getBytes().length == 1) {
			buffer.append(firstKey.toUpperCase() + CharacterNameParser.ValueType_ENG + firstKey);
		}
		else {
			try {
				String value = convert(firstKey);
				if (TextUtils.isEmpty(value)) {
					buffer.append("|" + CharacterNameParser.ValueType_CHS);
				}
				else {
					buffer.append(value.substring(0, 1).toUpperCase() + CharacterNameParser.ValueType_CHS + value);
				}
			}
			catch (Exception e) {
				buffer.append("|" + CharacterNameParser.ValueType_CHS);
		...
		for (int i = 1; i < chs.length(); i++) {
			...
			if (key.getBytes().length >= 2) {
				value = convert(key);
				if (value == null) {
					value = CharacterNameParser.ValueUnknown;
				}
			}
			else {
				value = key;
