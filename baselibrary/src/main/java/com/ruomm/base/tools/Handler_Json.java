...
public class Handler_Json {
	...
	private static Object JsonToHashMap(String str) {
		...
		try {
			...
			Log.d("JSON Parse Error", "错误字符串：" + str);
			return str;
		}
	}
		...
		try {
			...
			if (object instanceof JSONArray) {
				...
				if (root.length() > 0) {
					...
					Log.d("JSON Parse Error", "数组" + entity + "解析出错");
					// return null; //返回数组0
				}
				return list;
				...
				if (root.length() > 0) {
					...
					while (rootName.hasNext()) {
									...
									if (fieldEntity.clazz == null) {
										...
										if (classes.contains(value_class)) {
											...
											for (int i = 0; i < array.length(); i++) {
												...
												if (fieldEntity.clazz == long.class) {
													obj = Long.valueOf(array.get(i).toString());
												}
												list.add(obj);
											}
											obj = list;
						...
						if (!isHas) {
							Log.d("JSON Parse Error", "字段" + name + "在实体类" + entity + "不存在");
						}
					}
				}
				else {
					Log.d("JSON Parse Error", "数据长度不对 解析出错");
				}
				return entity;
			}
			else {
