...
package com.ruomm.base.zxing.camera;
...
public final class CameraManager {
	private static final float FRAMINGRECT_PERCENT = 0.6f;
	private static final float FRAMINGRECT_USER_PERCENT = 0.75f;
	...
	public Rect getFramingRect() {
		...
		if (framingRect == null) {
			...
			if (screenResolution.x < screenResolution.y) {
				// size = screenResolution.x;
				// size = size * screenResolution.x / screenResolution.y * 3 /
				// 4;
				size = (int) (screenResolution.x * FRAMINGRECT_PERCENT);
			}
			else {
				// size = screenResolution.y;
				// size = size * screenResolution.y / screenResolution.x * 3 /
				// 4;
				size = (int) (screenResolution.y * FRAMINGRECT_PERCENT);
			}
	...
	//
	public int getFramingHeight() {

		DisplayMetrics dm = context.getResources().getDisplayMetrics();
		int size = 0;
		if (dm.widthPixels < dm.heightPixels) {
			size = (int) (dm.widthPixels * FRAMINGRECT_PERCENT);
		}
		else {
			size = (int) (dm.heightPixels * FRAMINGRECT_PERCENT);
		}
		return size;
	}

	private Rect getFramingRectUser() {
		Point screenResolution = configManager.getScreenResolution();
		int size = 0;

		if (screenResolution.x < screenResolution.y) {
			size = (int) (screenResolution.x * FRAMINGRECT_USER_PERCENT);
		}
		else {
			size = (int) (screenResolution.y * FRAMINGRECT_USER_PERCENT);
		}

		int leftOffset = (screenResolution.x - size) / 2;
		int topOffset = (screenResolution.y - size) / 2;
		return new Rect(leftOffset, topOffset, leftOffset + size, topOffset + size);
	}
	...
	public Rect getFramingRectInPreview() {
		if (framingRectInPreview == null) {
			Rect rect = new Rect(getFramingRectUser());
