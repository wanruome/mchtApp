...
package com.ruomm.base.view.dragsortlistview;
...
public class DragSortListView extends ListView {
	 ...
	 * 
	 * @param which
	 *            Position to remove (NOTE: headers/footers ignored! this is a position in your
	 *            input ListAdapter).
	 * @param velocityX
	 */
	 ...
	 * 
	 * @param from
	 *            Position to move (NOTE: headers/footers ignored! this is a position in your input
	 *            ListAdapter).
	 * @param to
	 *            Target position (NOTE: headers/footers ignored! this is a position in your input
	 *            ListAdapter).
	 */
	 ...
	 * 
	 * @param remove
	 *            Remove the dragged item from the list. Calls a registered RemoveListener, if one
	 *            exists. Otherwise, calls the DropListener, if one exists.
	 ...
	 * @param deltaX
	 *            Offset in x of the touch coordinate from the left edge of the floating View (i.e.
	 *            touch-x minus float View left).
	 * @param deltaY
	 *            Offset in y of the touch coordinate from the top edge of the floating View (i.e.
	 *            touch-y minus float View top).
	...
	public interface FloatViewManager {
		 ...
		 * 
		 * @param position
		 *            Position of item to drag (NOTE: <code>position</code> excludes header Views;
		 *            thus, if you want to call {@link ListView#getChildAt(int)}, you will need to
		 ...
		 *            The floating View.
		 * @param location
		 *            The location (top-left; relative to DSLV top-left) at which the float View
		 *            would like to appear, given the current touch location and the offset provided
	 ...
	 * constant in time. Create your own by implementing {@link DragScrollProfile}.
	 * 
	 * @param ssp
	 */
	 ...
	 * items changes in an underlying adapter which does not have stable IDs (see
	 * {@link ListAdapter#hasStableIds()}). This is because without IDs, the ListView has no way of
	 * knowing which items have moved where, and cannot update the check state accordingly.
	 * <p>
	 ...
	 * of items changes in an underlying adapter which does not have stable IDs (see
	 * {@link ListAdapter#hasStableIds()}). This is because without IDs, the ListView has no way of
	 * knowing which items have moved where, and cannot update the check state accordingly. See also
	 * further comments on {@link #moveCheckState(int, int)}.
	 * 
	 * @param position
	 */
	...
	public interface DragScrollProfile {
		 ...
		 * 
		 * @param w
		 *            Normalized position in scroll region (i.e. w \in [0,1]). Small w typically
		 *            means slow scrolling.
		 * @param t
		 *            Time (in milliseconds) since start of scroll (handy if you want scroll
		 *            acceleration).
