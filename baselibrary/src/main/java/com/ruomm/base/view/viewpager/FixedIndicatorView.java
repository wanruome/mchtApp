...
import com.ruomm.base.view.viewpager.ScrollBar.Gravity;
...
public class FixedIndicatorView extends LinearLayout implements Indicator {
	...
	public void setCurrentItem(int item, boolean anim) {
		...
		if (mSelectedTabIndex != item) {
			...
			initNotifyOnPageScrollListener();
			if (getWidth() != 0 && anim && mPositionOffset < 0.01f && mPreSelectedTabIndex >= 0
	...
	private final Set<Integer> hasSelectPosition = new HashSet<Integer>(4);
	...
	private int measureScrollBar(int unSelect, int select, float selectPercent, boolean needChange) {
		...
		if (view.isLayoutRequested() || needChange) {
			if (mAdapter != null && mAdapter.getCount() > 0 && mSelectedTabIndex >= 0
				...
				if (selectV != null) {
					int width = (int) (selectV.getWidth() * selectPercent + (unSelectV == null ? 0 : unSelectV
							.getWidth() * (1 - selectPercent)));
