 ...
 *******************************************************************************/
package com.ruomm.base.view.pulltorefresh.library;
...
import com.ruomm.base.view.pulltorefresh.library.internal.FlipLoadingLayout;
import com.ruomm.base.view.pulltorefresh.library.internal.LoadingLayout;
import com.ruomm.base.view.pulltorefresh.library.internal.RotateLoadingLayout;
import com.ruomm.base.view.pulltorefresh.library.internal.Utils;
import com.ruomm.base.view.pulltorefresh.library.internal.ViewCompat;
import com.ruomm.base.view.xlistview.XListUtil;
import com.ruomm.R;
...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
	...
	public static final int SMOOTH_SCROLL_LONG_DURATION_MS = 650;
	static final int DEMO_SCROLL_INTERVAL = 600;
	...
	private Mode finishRefreshingMode = null;
	T mRefreshableView;
	...
	private void setRefreshState(final boolean... params) {
		mState = State.RESET;
		if (DEBUG) {
			Log.d(LOG_TAG, "State: " + mState.name());
		}
		onReset();
		if (null != finishRefreshingMode) {
			setMode(finishRefreshingMode);
		}
	...
	// this.getLoadingLayoutProxy().setLastUpdatedLabel(XListUtil.getRefreshTime(updateTime));
	// }
	...
	private void setRefreshTime() {
		if (null != mHeaderLayout) {
			if (isShowRefreshTime) {
				mHeaderLayout.setLastUpdatedLabel(XListUtil.getRefreshTime(this.refreshTime));
			}
			else {
				mHeaderLayout.setLastUpdatedLabel(null);
			}
		}
	}
	...
	private void setLoadingTime() {
		if (null != mFooterLayout) {
			if (isShowLoadingTime) {
				mFooterLayout.setLastUpdatedLabel(XListUtil.getLoadingTime(this.loadingTime));
			}
			else {
				mFooterLayout.setLastUpdatedLabel(null);
			}
		}
	}
	...
	protected void onReset() {
		...
		if (null != finishRefreshingMode) {
			if (finishRefreshingMode != mMode) {

				if (DEBUG) {
					Log.d(LOG_TAG, "Setting mode to: " + finishRefreshingMode);
				}
				mMode = finishRefreshingMode;
				finishRefreshingMode = null;
				isUpdateUI = true;

			}
		}
		if (null != finishShowComplete) {
			if (finishShowComplete != isShowComplete) {
				if (DEBUG) {
					Log.d(LOG_TAG, "Setting showComplte to: " + finishShowComplete);
				}
				isShowComplete = finishShowComplete;
				finishShowComplete = null;
				isUpdateUI = true;
			}
		}
	...
	protected void updateUIForMode() {
		...
		if (getPullToRefreshScrollDirection() == Orientation.VERTICAL) {
			if (null == completeView && isShowComplete) {
				completeView = LayoutInflater.from(getContext()).inflate(R.layout.pull_to_refresh_tag, null);

				addViewInternal(completeView, new LayoutParams(LayoutParams.MATCH_PARENT,
						LayoutParams.WRAP_CONTENT));

				completeText = (TextView) completeView.findViewById(R.id.completeTV);
				completeView.setOnClickListener(completeOnClickListener);

			}

			if (null != completeView && !isShowComplete) {
				completeView.setOnClickListener(null);
				removeView(completeView);
				completeView = null;
				completeText = null;
			}

			if (mMode.showFooterLoadingLayout() && null != completeText) {
				completeText.setText(R.string.xlistview_footer_hint_normal);
			}
			else if (null != completeText) {
				completeText.setText(R.string.xlistview_footer_hint_nomoredata);
			}
		...
		if (mMode.showFooterLoadingLayout()) {
			addViewInternal(mFooterLayout, new LayoutParams(LayoutParams.MATCH_PARENT,
					DisplayUtil.dipTopx(getContext(), 90)));
		}
	...
	public void setShowComplete(boolean isShowComplete) {
		if (isRefreshing()) {
			this.finishShowComplete = isShowComplete;
			updateUIForMode();
		}
		else {
			this.isShowComplete = isShowComplete;
			if (DEBUG) {
				Log.d(LOG_TAG, "Setting showComplte to: " + this.isShowComplete);
			}
			updateUIForMode();
		}
	}

	private void setCompleteStatus(boolean flag) {
	...
	private final OnClickListener completeOnClickListener = new OnClickListener() {
