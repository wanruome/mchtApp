...
public class WheelView extends View {
	...
	// 适配器
	private WheelStringAdapter mWheelAdapter;
	// View的高度和宽度以及中心的位置
	...
	// 需要绘制的Item的集合
	private final LinkedList<WheelStringItemValue> listItemValues = new LinkedList<WheelStringItemValue>();
	// 监听事件
	private final LinkedList<OnWheelStringChangedListener> changingListeners = new LinkedList<OnWheelStringChangedListener>();
	...
	public WheelView(Context context, AttributeSet attrs, int defStyleAttr) {
		...
		// text_rowSpacing = a.getFloat(R.styleable.WheelView_wheel_text_rowSpacing,
		// text_rowSpacing);
		...
		// 声明画笔
		// paint_reference = new Paint();
		// paint_reference.setAntiAlias(true);
		// paint_reference.setColor(Color.BLACK);
		// paint_reference.setTextSize(textsize);
		// paint_reference.setStyle(Paint.Style.STROKE);
	...
	// 设置适配器
	public void setAdapter(WheelStringAdapter wheelAdapter) {
	...
	public WheelStringAdapter getAdapter() {
		return mWheelAdapter;
	}

	// 设置监听
	public void addChangingListener(OnWheelStringChangedListener listener) {
		changingListeners.add(listener);
	}

	public void removeChangingListener(OnWheelStringChangedListener listener) {
		changingListeners.remove(listener);
	}

	protected void notifyChangingListeners(int oldValue, int newValue) {
		for (OnWheelStringChangedListener listener : changingListeners) {
	...
	private void calCurrentValue() {
		...
		WheelStringItemValue value_top;
		WheelStringItemValue value_buttom;
		if (current_y >= size_height / 2) {
			// 计算底部坐标系
			value_buttom = new WheelStringItemValue();
			value_buttom.index = currentItem;
			value_buttom.y_postion = current_y;
			// 计算顶部坐标系
			value_top = new WheelStringItemValue();
	...
	private void calValues_Top(WheelStringItemValue value_temp) {
		...
		while (ytemp > 0) {
			WheelStringItemValue value = new WheelStringItemValue();
	...
	private void calValues_buttom(WheelStringItemValue value_temp) {
		float ytemp = value_temp.y_postion + value_temp.text_size * text_lineSpacing;
		int temp_index = value_temp.index + 1;
		if (isCyclic) {
	...
	// 计算每个Item的参数
	private void calItemValue(WheelStringItemValue value_cal) {
	...
	private void drawItemsValue(Canvas canvas) {
		...
		for (WheelStringItemValue value : listItemValues) {
			...
			float[] temps = new float[value.text_value.length()];
			paint_text.getTextWidths(value.text_value, temps);
			float tempwidth = 0;
			for (float temp : temps) {
				tempwidth += temp;
			}
			float base_y = value.y_postion + value.text_size * Value_PaintText_Offset / 2;
			float base_x = center_x - tempwidth / 2;
			...
			paint_text.setTextSkewX(value.skewx);
