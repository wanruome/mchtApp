...
package com.ruomm.base.view.staggeredgridview;
 ...
 * <p/>
 * This is partly inspired by the incomplete StaggeredGridView supplied in the Android 4.2+ source &
 * the {@link AbsListView} & {@link android.widget.ListView} source; however this is
 * intended to have a smaller simplified scope of functionality and hopefully therefore be a
 * workable solution.
 * <p/>
 * Some things that this doesn't support (yet) - Dividers (We don't use them in our Etsy grid) -
 * Edge effect - Fading edge - yuck - Item selection - Focus
 * <p/>
 * Note: we only really extend {@link AbsListView} so we can appear to be one of its
 * direct subclasses. However most of the code we need to modify is either 1. hidden or 2. package
 * private So a lot of it's code and some {@link AdapterView} code is repeated here
 * Be careful with this - not everything may be how you expect if you assume this to be a regular
 * old {@link android.widget.ListView}
 */
public abstract class ExtendableListView extends AbsListView {
	...
	protected void onLayout(final boolean changed, final int l, final int t, final int r, final int b) {
		// super.onLayout(changed, l, t, r, b); - skipping base AbsListView implementation on
		// purpose
	...
	private boolean onTouchUpScrolling(final MotionEvent event) {
		if (hasChildren()) {
			...
			final boolean atEdge = mFirstPosition == 0 && top >= getListPaddingTop()
					&& mFirstPosition + getChildCount() < mItemCount && bottom <= getHeight() - getListPaddingBottom();
	 ...
	 * that represents a big enough scroll.
	 */
	 ...
	 * @param top
	 *            Pixel offset from the top of this view to the top of the reference view.
	 ...
	 * @param flowDown
	 *            If true, align top edge to y. If false, align bottom edge to y.
	 * @param selected
	 *            Is this position selected?
	 * @param recycled
	 *            Has this view been pulled from the recycle bin? If so it does not need to be
	 *            remeasured.
	 */
	 ...
	 *            was taken from the scrap heap, false if otherwise.
	...
	/**
	 * Check if we have dragged the bottom of the list too high (we have pushed the top element off
	...
	/**
	 * Check if we have dragged the bottom of the list too low (we have pushed the bottom element
	 * off the bottom of the screen when we did not need to). Correct by sliding everything back up.
	 * 
	 * @param childCount
	 *            Number of children
	 */
	...
	/**
	 * Re-implementing some properties in {@link ViewGroup.LayoutParams} since they're
	 * package private but we want to appear to be an extension of the existing class.
	 */
