...
public abstract class BasePopupWindowUserConfig extends PopupWindow {
	...
	public BasePopupWindowUserConfig(Context mContext, int layoutId, BaseDialogClickListener listener) {
		super(mContext);
		this.popContext = mContext;
		this.listener = listener;
		LayoutInflater inflater = (LayoutInflater) mContext.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
		mview = inflater.inflate(layoutId, null);
		this.setWidth(getLayoutParam_Width());
		// 设置SelectPicPopupWindow弹出窗体的高
		this.setHeight(getLayoutParam_Height());
	...
	public void setImageResource(int vID, int imageId) {
		ImageView v = (ImageView) findViewById(vID);
